...
import {
  LayoutDashboard, ClipboardList, FolderOpen, HeartHandshake, Settings, 
  Sun, Moon, LogOut, Bell, CheckCheck
} from 'lucide-react';
...
export default function SpecialNeedsDashboard() {
  ...
  const NAV = [
    ...
    { id: 'support',   label: 'Support/Help',  icon: HeartHandshake },
            ...
            {NAV.map(item => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    if (item.id === 'settings') {
                      navigate('/settings'); // تعديل هذا المسار لاحقاً لصفحة الإعدادات الخاصة بهم
                    } else {
                      setTab(item.id);
                      setSidebar(false);
                    }
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className={`h-16 flex-shrink-0 flex items-center justify-between px-6 border-b ${topbarBg}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => navigate('/profile')} className="w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold text-white overflow-hidden ml-1" style={{ background: NAVY }}>
              {user?.avatar ? <img src={user.avatar} className="w-full h-full object-cover" alt="" /> : initials(userName)}
            </button>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <AnimatePresence mode="wait">
            ...
            {/* التبويبات القادمة التي سنبرمجها (حالياً نضع رسالة مؤقتة) */}
            {activeTab === 'requests' && <div key="requests">My Requests coming soon...</div>}
